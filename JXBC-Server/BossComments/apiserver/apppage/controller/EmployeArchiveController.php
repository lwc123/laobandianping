...
class EmployeArchiveController extends CompanyApiController {
	public function Archive(Request $request) {
		...
		if ($Report) {
            ...
            //修改记录
            $Report ['loglist']=ArchiveCommentLog::where('CommentId',$Report ['CommentId'])->find();
		} else {
			$Report = '';
		}
			...
			foreach ( $StageYearlist as $k => $v ) {
				...
				foreach ( $StageYear [$v] ['Commentlist'] as $key => $val ) {
					...
					if ($StageYear [$v] ['Commentlist'] [$key]  ['WorkCommentImages']) {
						...
						$WorkCommentImages=[];
						foreach ( $StageYear [$v] ['Commentlist'] [$key] ['WorkCommentImages'] as $value ) {
							$WorkCommentImages [] = Config ( 'resources_site_root' ) . $value;
						}
                    ...
                    //修改记录
                    $StageYear [$v] ['Commentlist'] [$key] ['loglist']=ArchiveCommentLog::where('CommentId',$val ['CommentId'])->find();
				}
			}
		} else {
			$StageYearlist = '';
	...
	public function Comment(Request $request) {
        ...
        //修改记录
        $comment ['loglist']=ArchiveCommentLog::where('CommentId',$oid)->find();
        // 档案详情
