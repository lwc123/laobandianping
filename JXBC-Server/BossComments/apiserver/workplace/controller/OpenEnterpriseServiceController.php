 ...
 * )
 */ 
class OpenEnterpriseServiceController extends PaymentHandlerController {
	/**
	 * @SWG\POST(
	 *     path="/workplace/OpenEnterpriseService/Preprocess",
	 *     summary="企业开户前的预处理",
	...
	public function Preprocess(Request $request) {
        ...
        $openEnterpriseRequest = json_decode($payment["CommodityExtension"]);  
        
        Log::record('CompanyName:'.$openEnterpriseRequest->CompanyName);
        
        if(empty($openEnterpriseRequest) || empty($openEnterpriseRequest->CompanyName)) {
            exception('非法请求 CompanyName', 412);
        }
        
        $company = Company::get(['CompanyName' => $openEnterpriseRequest->CompanyName]);
        if(empty($company)) {
            return true;
        } else {
            return false;
    ...
    /**    
     * @SWG\POST(
     *     path="/workplace/OpenEnterpriseService/BizProcess",
     *     summary="企业开户",     
     *     tags={"PaymentBizProcessor"},
     *     @SWG\Parameter(
     *         name="body",
    ...
    public function BizProcess(Request $request) {
        ...
        try{
            $company = EnterpriseService::openEnterpriseService($tradeJournal);
