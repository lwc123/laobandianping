...
class EnterpriseServiceController extends AuthenticatedController
{
    ...
    public function RenewalService(Request $request)
    {
        ...
        $Member=  CompanyMember::getPassportRoleByCompanyId($companyId, $this->PassportId);
        if(empty($Member['MobilePhone'])){
            $Member=  CompanyMember::where(['CompanyId' => $companyId, "PassportId" => $this->PassportId])->find();
        }
        $this->assign('PriceStrategy', $priceStrategy);
    ...
    {
        $priceStrategy = PriceStrategyService::CurrentActivity(array('ActivityType'=>1));
