...
class Request
{
    ...
    public function get($name = '', $default = null, $filter = null)
    {
        ...
        //fixed:支持url参数名，忽略首字母大小写
        $value = $this->input($this->get, $name, $default, $filter);
        if(!empty($name) && !isset($value)) {
            $fisrtChar = substr($name,0,1);
            if(strtolower($fisrtChar)==$fisrtChar) {
                $value = $this->input($this->get, ucfirst($name), $default, $filter);
            } else {
                $value = $this->input($this->get, lcfirst($name), $default, $filter);
            }
        }
        return $value;
    }
