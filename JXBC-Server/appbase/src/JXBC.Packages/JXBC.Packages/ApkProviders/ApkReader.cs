...
namespace JXBC.Packages.ApkProviders
{
    public class ApkReader
    {
        ...
        public ApkInfo extractInfo(byte[] manifest_xml, byte[] resources_arsx)
        {
            ...
            try
            {
                var s = System.Text.Encoding.Default.GetString(manifest_xml);
                manifestXml = manifest.ReadManifestFileIntoXml(manifest_xml);//AppReport.Analysis.AndroidXmlDecompress.DecompressXML(manifest_xml);//
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public ApkInfo extractInfo(XmlDocument manifestXml, byte[] resources_arsx)
        {
            ...
            try
            {
                ...
                info.metaData = FindMetaData(doc.DocumentElement);
        ...
        private IDictionary<string, string> FindMetaData(XmlElement element)
        {
            var targetNodes = element.GetElementsByTagName("meta-data");
            IDictionary<string, string> metaData = new Dictionary<string, string>();
            if (null == targetNodes || 0 == targetNodes.Count) return metaData;

            foreach (XmlNode node in targetNodes)
            {
                if (null == node.Attributes || node.Attributes.Count != 2)
                    return metaData;
                var name = node.Attributes.GetNamedItem("name") == null ? "" : node.Attributes.GetNamedItem("name").Value;
                var value = node.Attributes.GetNamedItem("value") == null ? "" : node.Attributes.GetNamedItem("value").Value;
                //metaData.Add(node.Attributes.GetNamedItem("name").Value, node.Attributes.GetNamedItem("value").Value);
