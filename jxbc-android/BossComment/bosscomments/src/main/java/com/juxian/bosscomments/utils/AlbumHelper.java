...
public class AlbumHelper {
		...
		if (cur.moveToFirst()) {
			...
			do {
				...
				if (bucket == null) {
					...
					bucket.imgpath = new File(path).getParent();
				}
	...
	public List<ImageBucket> getImagesBucketList(boolean refresh) {
		...
		String caramPath = FileUtil.getCameraPath();

		Iterator<Entry<String, ImageBucket>> itr = bucketList.entrySet()
				.iterator();
		for (Entry<String, ImageBucket> entry : bucketList.entrySet()) {
			if (entry.getValue().imgpath.equals(caramPath)) {
				tmpList.add(entry.getValue());
			}
		}
		for (Entry<String, ImageBucket> entry : bucketList.entrySet()) {
			if (false == entry.getValue().imgpath.equals(caramPath)) {
				tmpList.add(entry.getValue());
			}
		}
