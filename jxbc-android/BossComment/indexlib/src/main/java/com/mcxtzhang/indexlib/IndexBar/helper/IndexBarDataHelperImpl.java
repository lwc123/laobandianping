...
public class IndexBarDataHelperImpl implements IIndexBarDataHelper {
    ...
    public IIndexBarDataHelper convert(List<? extends BaseIndexPinyinBean> datas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                ...
                String str = initPolyphone(pySb.toString());//多音字处理
                indexPinyinBean.setBaseIndexPinyin(str);//设置城市名全拼音
            } else {
    ...
    private String initPolyphone(String str) {
        if ("ZHONGQING".equals(str)) {
            str = "CHONGQING";//重庆
        }
        if ("ZHANGSHA".equals(str)) {
            str = "CHANGSHA";//长沙
        }
        if ("DACHENG".equals(str)) {
            str = "DAICHENG";//大城
        }
        if ("WEIXIAN".equals(str)) {
            str = "YUXIAN";//蔚县
        }
        if ("YUETING".equals(str)) {
            str = "LAOTING";//乐亭
        }
        if ("LEQING".equals(str)) {
            str = "YUEQING";//乐清
        }
        if ("WOYANG".equals(str)) {
            str = "GUOYANG";//涡阳
        }
        if ("QIANSHAN".equals(str)) {
            str = "YANSHAN";//铅山
        }
        if ("DANXIAN".equals(str)) {
            str = "SHANXIAN";//单县
        }
        if ("XINXIAN".equals(str)) {
            str = "SHENXIAN";//莘县
        }
        if ("MIYANG".equals(str)) {
            str = "BIYANG";//泌阳
        }
        if ("JUNXIAN".equals(str)) {
            str = "XUNXIAN";//浚县
        }
        if ("FANYU".equals(str)) {
            str = "PANYU";//番禺
        }
        if ("YUNLIAN".equals(str)) {
            str = "JUNLIAN";//筠连
        }
        if ("JIANWEI".equals(str)) {
            str = "QIANWEI";// 犍为
        }
        if ("WEILI".equals(str)) {
            str = "YULI";//尉犁
        }
        if ("SHAMEN".equals(str)) {
            str = "XIAMEN";//尉犁
        }
        return str;
    }
