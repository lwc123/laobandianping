...
- (void)setUpSubViews
{
    ...
    [self.change_Btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if(timeout<=0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                self.getCodeButton.backgroundColor = [PublicUseMethod setColor:KColor_CodeColor];
                [self.getCodeButton setBackgroundImage:nil forState:UIControlStateNormal];
            });
            timeout--;
        }
    });
...
- (void)endTimer
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.getCodeButton.userInteractionEnabled = YES;
        [self.getCodeButton setBackgroundImage:[UIImage imageNamed:@"codebutton"] forState:UIControlStateNormal];
    });
