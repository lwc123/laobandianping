...
+(void)createNew:(void(^)(AccountSignResult* result))success fail:(void(^)(NSError* error))fail
{
    ...
    [WebAPIClient postJSONWithUrl:API_Account_CreateNew parameters:dic
                          success:^(id result)
     {
          AccountSignResult *resultEntity = [[AccountSignResult alloc] initWithDictionary:result error:nil];
...
+ (void)changeCurrentToUserProfileSuccess:(void (^)(AccountEntity* result))success fail:(void (^)(NSError* error))fail{
    ...
    [WebAPIClient postJSONWithUrl:API_Account_ChangeCurrentToUserProfile parameters:nil success:^(id result) {
        Log(@"%@",result);
        if (![result isKindOfClass:[NSNull class]]) {
            AccountEntity *signResult = [[AccountEntity alloc]initWithDictionary:result error:nil];
            
            AnonymousAccount * account = [UserAuthentication GetCurrentAccount];
            account.UserProfile.CurrentProfileType = UserProfile;
...
// 切换到企业端
+ (void)changeCurrentToOrganizationProfileWithprofile:(JXOrganizationProfile *)organizationProfile success:(void (^)(AccountEntity *))success fail:(void (^)(NSError *))fail{

    [WebAPIClient postJSONWithUrl:API_Account_ChangeCurrentToOrganizationProfile parameters:[organizationProfile toDictionary] success:^(id result) {
