...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (_firstIn) {
        [self.collectionView setContentOffset:CGPointMake(0,(((screenWidth - 3)/4)+1)*(self.assets.count/4 + 2) - self.collectionView.height) animated:NO];
    }
...
- (void)photoLibraryDidChange:(PHChange *)changeInstance {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        // Get the new fetch result. fetchResultBeforeChanges
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    #pragma mark - jam
    PHAsset *asset = self.assets[self.assets.count - indexPath.item - 1];
...
//        PHAsset *asset = self.assets[index];
        PHAsset *asset = self.assets[self.assets.count - 1 - index];
