...
- (void)showWithImageViews:(NSArray*)views selectedView:(UIImageView*)selectedView {
    ...
    self.pageControl.currentPage = [views indexOfObject:selectedView];
...
- (void)prepareToDismiss {
    ...
    for(UIImageView *view in _imgViews) {
        if(view != currentView) {
            ...
            #pragma mark - jam
            [state.superview sendSubviewToBack:view];
        }
    }
}
...
#pragma mark - jam
                         [state.superview sendSubviewToBack:currentView];
...
- (UIPageControl *)pageControl{
    ...
    if (_pageControl == nil) {
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, ScreenHeight-40, ScreenWidth, 20)];
