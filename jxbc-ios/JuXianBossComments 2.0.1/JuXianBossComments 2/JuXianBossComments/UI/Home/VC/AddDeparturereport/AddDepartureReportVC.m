...
- (void)initData{
    ...
    NSString * bossStr = [NSString stringWithFormat:@"%@(老板)",_bossMembe.RealName];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {//选择员工档案
        ...
        if (indexPath.row == 0) {
            ...
            textCell.jhtextfield.placeholder = @"请输入离任时间";
        ...
        NSString * myStr = @"添加离任报告审核人(一人通过审核即生效)";
...
- (void)textViewDidChange:(UITextView *)textView{
    if (textView.text.length > 500) {
        ...
        //SC.XJH.1.4
        _detaiComment.WorkComment = _mytexiView.text;
    }
