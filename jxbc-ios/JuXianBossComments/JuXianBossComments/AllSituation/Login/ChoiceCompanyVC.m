...
- (void)viewDidLoad {
    ...
    if ([self.secondVC isKindOfClass:[MineAccountViewController class]] || [self.secondVC isKindOfClass:[UserMineVC class]]) {
    ...
    if (company.AuditStatus == 2) { // 已通过
        ...
        if (!company.getServiceDays || company.getServiceDays >= 30) { // 未到期
            ...
            if (company.getServiceDays < 0) { // 到期
                ...
                companyCell.moneyLabel.text = [NSString stringWithFormat:@"剩余%zd天",company.getServiceDays +1];
            }
