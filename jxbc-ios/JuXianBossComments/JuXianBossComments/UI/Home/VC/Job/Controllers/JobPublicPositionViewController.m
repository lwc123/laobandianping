...
- (void)rightButtonAction:(UIButton* )sender{
    ...
    if (self.isUpdate) {// 修改
        ...
        [WorkBenchJobRequest postJob_publicJobWith:self.jobEntity success:^(id result) {
            ...
            if ([self.navigationController.viewControllers[1] isKindOfClass:[JobPublicListViewController class]]) {
                
                JobPublicListViewController * job = self.navigationController.viewControllers[1];
...
- (NSArray *)titleArray{
    ...
    if (!_titleArray) {
        _titleArray = @[@"公司名称",@"职位名称",@"薪资范围",@"经验要求",@"学历要求",@"工作城市",@"工作地点",@"职位描述",@"接收简历邮箱",@"联系电话"];
    }
    return _titleArray;
}
- (JXFooterView*)footerView{
    JXFooterView* footerView = [JXFooterView footerView];
    footerView.nextLabel.text = self.isUpdate ? @"保存":@"发布";
