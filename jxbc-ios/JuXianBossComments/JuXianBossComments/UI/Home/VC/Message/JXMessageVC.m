...
- (void)viewDidLoad {
    ...
    //一进来就请求待处理消息
    _messageType = 2;
    [self redPointWithMessagetype:1];
    [self redPointWithMessagetype:2];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (_messageType) {
        [self redPointWithMessagetype:_messageType];
    }
}
...
- (void)redPointWithMessagetype:(int)messagetype{
    MJWeakSelf
    [self showLoadingIndicator];
    [WorkbenchRequest getMessageUnreadWithmessageType:messagetype success:^(id result) {
        [weakSelf dismissLoadingIndicator];
        
        if (messagetype == 2) {//待处理
            
            if ([result integerValue] > 0) {//fase 没有未读消息
                _waitsShow = YES;
            }else{
                _waitsShow = NO;
            }
