...
- (void)initData{
    ...
    NSString * bossStr = [NSString stringWithFormat:@"%@(法人)",_membeEntity.RealName];
    ...
    if (_detailComment.CommentId > 0) {
        ...
        self.mytexiView.text = _detailComment.WorkComment;
    }
    ...
    if (self.detailComment.AuditPersonList!=nil) {
        ...
        for (NSDictionary *dict in _detailComment.AuditPersonList) {
            ...
            if (auditPersonModel.Role == Role_Boss) {
                [_reviewPeopleArray insertObject:[NSString stringWithFormat:@"%@(%@)",auditPersonModel.RealName,auditPersonModel.JobTitle] atIndex:0];
            }else{
...
- (void)leftButtonAction:(UIButton *)button{
    if ([self.secondVC isKindOfClass:[AddStaffRecordVC class]]) {//从添加档案过来的
        ...
        if (!_detailComment.CommentId) {//修改不需要提示
            
            if (self.archiveId > 0 || _nameStr != nil || self.checkTimeStr != nil || _abilityScore > 0 || _attitudeScore > 0 || _achievementScore > 0 || _mytexiView.text.length != 0 || _mytexiView.text.length > 0) {
                [self alertWithTitle:@"温馨提示" message:@"信息尚未保存，是否离开？" cancelTitle:@"否" okTitle:@"是" CommentEntity:nil];
            }else{
                [self.navigationController popViewControllerAnimated:YES];
            }
        }else{
