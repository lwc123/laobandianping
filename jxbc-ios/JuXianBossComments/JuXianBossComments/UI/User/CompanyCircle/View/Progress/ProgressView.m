...
- (void)addArcBackColor{
    //没画的
    CGColorRef color = (_arcBackColor == nil) ? [UIColor purpleColor].CGColor : _arcBackColor.CGColor;
    ...
    // Draw the slices.
    CGFloat radius = viewSize.width / 2 - 1;
...
- (void)drawArc{
    ...
    if (_percent == 1) {
        ...
        CGPoint center = CGPointMake(viewSize.width / 2, viewSize.height / 2);
        // Draw the slices.
        CGFloat radius = viewSize.width / 2 + 3;
        CGContextBeginPath(contextRef);
        CGContextMoveToPoint(contextRef, center.x, center.y);
...
-(void)addCenterBack{
    ...
    //里面的颜色
    CGColorRef color = (_centerColor == nil) ? [UIColor yellowColor].CGColor : _centerColor.CGColor;
    CGContextRef contextRef = UIGraphicsGetCurrentContext();
    CGSize viewSize = self.bounds.size;
    CGPoint center = CGPointMake(viewSize.width / 2, viewSize.height / 2);
    // Draw the slices.
...
//        percent = [NSString stringWithFormat:@"%0.2f%%",_percent*100];
        percent = [NSString stringWithFormat:@"%1.f%%",_percent*100];
