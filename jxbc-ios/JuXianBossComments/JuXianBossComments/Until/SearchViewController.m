...
- (void)initUI{
    ...
    self.myTableView.sectionIndexColor = ColorWithHex(KColor_Text_BlackColor);
...
- (void)initRequestCity{
    ...
    [WebAPIClient getJSONWithUrl:API_Dictionary_City parameters:nil success:^(id result) {
        ...
        // 添加热门城市索引
        [weakSelf.letterArray insertObject:@"热" atIndex:0];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        if (self.hotCityArray.count>0) {
            return (self.hotCityArray.count/3 + 1) * (35 + 10) + 8;
        }else{
            return 0;
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        [cell addSubview:self.hotCityView];
        return cell;
    }
...
//            CGFloat cellY = row * (cellH + margin) + (CGRectGetMaxY(cityLabel.frame) + 8);
            CGFloat cellY = row * (cellH + margin) + 8;
